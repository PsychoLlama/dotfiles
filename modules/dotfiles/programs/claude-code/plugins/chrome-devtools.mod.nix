...
let
  chromiumExe = lib.getExe' self.programs.chromium.package "chromium";

  wrapper = pkgs.writeShellApplication {
    name = "chrome-devtools-mcp";
    text = ''
      if [ "''${CHROME_MCP_AUTOCONNECT:-}" = "true" ]; then
        args=(--autoConnect ${lib.optionalString pkgs.stdenv.isLinux ''--userDataDir "$HOME/.config/chromium"''})
      else
...
{
  config.programs.claude-code.marketplace.plugins.chrome-devtools = {
