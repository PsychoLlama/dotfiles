...
rhizome.plugin {
  ...
  # Declaring a class means routing it. On the node, so it holds whether
  # or not anything opted in. Fragments only exist in the fixpoint the
  # mount evaluated in, so this is `nixos`: on a host they land an eval
  # above the editor, and `sharedModules` is the way down.
  node = {
    modules.nixos =
      {
        config,
