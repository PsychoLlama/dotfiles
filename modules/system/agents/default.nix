...
#
# Declared on the flake. `generic` was the previous answer to the same problem:
# the codex preset is a NixOS module and the claude-code preset a Home Manager
# one, and `generic` was the only class evaluated inside both. A flake option is
# read by either without belonging to a class at all.
